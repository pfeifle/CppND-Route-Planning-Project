...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
  ...
  for(RouteModel::Node *n : current_node->neighbors){
      ...
      else if(n->g_value > current_node->g_value +current_node->distance(*n)){
        n->parent  = current_node;
        n->g_value = current_node->g_value +current_node->distance(*n);
      }
   }
...
void RoutePlanner::ListSort(std::vector<RouteModel::Node*> *v){
    sort(v->begin(),v->end(),this->Compare);
}
...
RouteModel::Node *RoutePlanner::NextNode() {
    this->ListSort(&(this->open_list));
    auto p_next =this->open_list.back();
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    while (current_node != this->start_node){
        ...
        it = path_found.insert(it,*(current_node->parent));
